...
Therefore, we only show the results obtained from the de novo-assembled sequences in figure 5c of the main text.
...
These results are visualized in figure 5d of the main text.
...
Given the relatively modest average sequencing depth (12X) of the Oxford Nanopore data obtained in this study, we performed subsampling analyses of the data for four cultivars.
...
* SV calling with Sniffles using parameters \texttt{--min\_support 3 --min\_seq\_size 1000 --min\_homo\_af 0.7}
* Filtering of variants as described in the main text
* Benchmarking of each SV dataset thus called and filtered with the sveval R package, using the SVs called on the complete dataset for the corresponding cultivar as a ground truth
...
Results of the subsampling analyses are presented in \ref{fig_s2} (deletions), \ref{fig_s3} (insertions), \ref{fig_s4} (duplications) and \ref{fig_s5} (inversions).
...
% Figure Y \caption[Graphical summary of the analyses presented in the paper]{{{{
\begin{lsfigure}
...
\end{lsfigure}
	...
	\captionof{figure}[Graphical summary of the analyses presented in the paper]{
		Graphical summary of the analyses presented in the paper.
...
\begin{cfigure}
	...
	\caption[Effects of sequencing depth on the sensitivity and precision of deletion discovery from Oxford Nanopore sequencing data]{
		...
		SVs were called on each of these subsamples and (A) sensitivity and (B) computed by comparing the resulting SVs to the SVs called on the full dataset for the corresponding cultivar.
		Points indicate the median value for a given cultivar/subsampling fraction combination, while vertical error bars indicate the span from the minimum to the maximum value.
