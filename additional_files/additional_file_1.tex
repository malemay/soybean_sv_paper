...
Table \ref{table_s9} shows the number of variants that could be refined or not for each sample.
...
Given the relatively modest average sequencing depth (12X) of the Oxford Nanopore data used in this study, we performed subsampling analyses of the data for four cultivars.
We generated these results both to better understand our results and to provide guidelines for other researchers wanting to use Oxford Nanopore sequencing data for calling SVs.
...
Each of the 80 \texttt{.bam} files thus generated were processed as described in the main text to generate a catalog of SV calls from each of them:

\begin{itemize}
	\item SV calling with Sniffles using parameters \texttt{--min\_support 3 --min\_seq\_size 1000 --min\_homo\_af 0.7}
	\item Filtering of variants as described in the main text
	\item Benchmarking of each such filtered SV dataset with the sveval R package, using the SVs called on the complete dataset for the corresponding cultivar as a ground truth
\end{itemize}
...
In the context of this analysis, ``sensitivity'' represents the proportion of SVs found in a given subsample relative to the complete dataset.
...
The results obtained for deletions (figure \ref{fig_s6}) show that sensitivity reaches 80\% at a sequencing depth of 10X across all size classes.
The precision of deletion discovery is close to perfection across all size classes but the smallest one (50 - 100 bp) and does not vary with sequencing depth.
...
The results obtained for insertions (figure \ref{fig_s7}) show that sensitivity reaches 75\% at a sequencing depth of 10X for insertions in the range 50 - 1,000 bp but decreases for larger insertions.
Therefore, it appears that the effect of sequencing depth becomes more important as insertions increase in size.
The precision of insertion discovery varies between 85 and 95\% depending on insertion size, suggesting that insertion discovery is less accurate than deletion discovery.
However, we observe the same pattern of independence between sequencing depth and precision as was observed for deletions.
...
The results obtained for duplications (figure \ref{fig_s8}) show that duplication discovery is more sensitive to sequencing depth than other SV types, with sensitivity below 50\% at a sequencing depth of 10X.
...
This result is also consistent with our observation that the precision of duplication genotyping depends strongly on Oxford Nanopore sequencing depth (see figure \ref{fig_s5}).
The precision of duplication discovery seen in figure \ref{fig_s8} is also poor, consistenly with difficulties in discovering and genotyping duplications documented elsewhere in this paper.

Finally, the results obtained for inversions (figure \ref{fig_s9}) show sensitivity values intermediate between those observed for insertions and duplications, with sensitivity reaching between 50 and 75\% at a sequencing depth of 10X.
...
One exception to this result is smaller inversions (50 - 100 bp), as the small number of inversions observed in this size class (see table \ref{table_s1}) caused important stochastic variation in precision rates depending on the cultivar and subsample.
...
% Table X \caption{Metadata on the Illumina sequencing data of the 102 Canadian accessions}{{{
...
% Table S7 \caption{DNA extraction and preparation methods for Oxford Nanopore sequencing}{{{
\begin{table}
	\centering
...
% Figure S2 \caption[Sensitivity and precision of Illumina deletion and insertion genotyping in non-repeat regions]{{{{
\begin{cfigure}
	% DEPENDENCY : figure_s2.png
	\includegraphics[width = 5in]{figure_s2}
...
% Figure S3 \caption[Sensitivity and precision of Illumina duplication and inversion genotyping]{{{{
\begin{cfigure}
...
% Figure S4 \caption[Sensitivity and precision of Illumina duplication and inversion genotyping in non-repeat regions]{{{{
\begin{cfigure}
...
% Figure S5 \caption[Relationship between Oxford Nanopore sequencing depth and genotyping precision of Illumina SVs]{{{{
\begin{cfigure}
	% DEPENDENCY : figure_s5.png
	\includegraphics[width = 6in]{figure_s5}
...
% Figure s6 \caption[Effects of sequencing depth on the sensitivity and precision of deletion discovery from Oxford Nanopore sequencing data]{{{{
\begin{cfigure}
	% DEPENDENCY : figure_s6.png
	\includegraphics[width = 6.2in]{figure_s6}
...
% Figure s7 \caption[Effects of sequencing depth on the sensitivity and precision of insertion discovery from Oxford Nanopore sequencing data]{{{{
\begin{cfigure}
...
% Figure s8 \caption[Effects of sequencing depth on the sensitivity and precision of duplication discovery from Oxford Nanopore sequencing data]{{{{
\begin{cfigure}
...
% Figure s9 \caption[Effects of sequencing depth on the sensitivity and precision of inversion discovery from Oxford Nanopore sequencing data]{{{{
\begin{cfigure}
...
% Figure S10 \caption[Effect of homozygous ALT count on genotyping performance of Illumina insertions and deletions]{{{{
\begin{cfigure}
	% DEPENDENCY : figure_s10.png
	\includegraphics[width = 5in]{figure_s10}
...
% Figure S11 \caption[Effect of homozygous ALT count on genotyping performance of Illumina duplications and inversions]{{{{
\begin{cfigure}
	% DEPENDENCY : figure_s11.png
	\includegraphics[width = 3.5in]{figure_s11}
...
% Figure S12 \caption[Effect of the number of supporting calling tools on genotyping performance of Illumina deletions and insertions]{{{{
\begin{cfigure}
	% DEPENDENCY : figure_s12.png
	\includegraphics[width = 5in]{figure_s12}
...
% Figure S13 \caption[Size distribution of the SVs reported by various calling tools using Illumina data]{{{{
\begin{cfigure}
	% DEPENDENCY : figure_s13.png
	\includegraphics[width = 6.5in]{figure_s13}
...
% Figure S14 \caption[Number of true positive and false positive deletion calls reported by various calling tool combinations]{{{{
\begin{lsfigure}
	% DEPENDENCY : figure_s14.png
	\includegraphics[width = 9in]{figure_s14}
...
% Figure S15 \caption[Number of true positive and false positive insertion calls reported by various calling tool combinations]{{{{
\begin{cfigure}
	% DEPENDENCY : figure_s15.png
	\includegraphics[width = 6.5in]{figure_s15}
...
% Figure S16 \caption[Relationship between Oxford Nanopore read N50 and genotyping precision of Oxford Nanopore insertions]{{{{
\begin{lsfigure}
	% DEPENDENCY : figure_s16.png
	\includegraphics[width = 8in]{figure_s16}
...
% Figure S17 \caption[Sensitivity and precision of Oxford Nanopore deletion and insertion genotyping in non-repeat regions]{{{{
\begin{lsfigure}
...
% Figure S18 \caption[Sensitivity and precision of Oxford Nanopore duplication and inversion genotyping]{{{{
\begin{cfigure}
	% DEPENDENCY : figure_s18.png
	\includegraphics[width = 3.5in]{figure_s18}
...
% Figure S19 \caption[Sensitivity and precision of Oxford Nanopore duplication and inversion genotyping in non-repeat regions]{{{{
\begin{cfigure}
...
% Figure S20  \caption[Sensitivity and precision of joint Illumina/Oxford Nanopore deletion and insertion genotyping]{{{{
\begin{lsfigure}
	% DEPENDENCY : figure_s20.png
	\includegraphics[width = 9.5in]{figure_s20}
...
% Figure S21 \caption[Sensitivity and precision of joint Illumina/Oxford Nanopore deletion and insertion genotyping in non-repeat regions]{{{{
\begin{lsfigure}
...
% Figure S22 \caption[Principal component analyses (PCA) of the whole Canadian soybean panel]{{{{
\begin{cfigure}
	% DEPENDENCY : figure_s22.png
	\includegraphics[width = 6.5in]{figure_s22}
...
% Figure s23 \caption[Neighbour-joining phylogenetic trees of the population of 102 Canadian soybean cultivars]{{{{
\begin{lsfigure}
	% DEPENDENCY : figure_s23.png
	\includegraphics[width = 9.5in]{figure_s23}
...
% Figure S24 \caption[IGV screenshot of Oxford Nanopore and Illumina read alignments of three samples at the location of a polymorphic Stowaway TE insertion]{{{
\begin{lsfigure}
...
% Figure S25 \caption[Genotyping benchmark of raw and refined Oxford Nanopore SVs for four samples using BayesTyper]{{{
\begin{lsfigure}
	% DEPENDENCY : figure_s25.png
	\includegraphics[width = 9.5in]{figure_s25}
...
% Figure S26 \caption[Genotyping sensitivity of raw and refined Oxford Nanopore SVs for four samples using vg]{{{{
\begin{lsfigure}
...
% Figure S27 \caption[Genotyping sensitivity of raw and refined Oxford Nanopore SVs for four samples using Paragraph]{{{
\begin{lsfigure}
