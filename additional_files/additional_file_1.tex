...
In order to address this issue, we assembled a pipeline that can be used to refine the SVs discovered by Sniffles [24].
...
The pipeline performs a local assembly of the Oxford Nanopore reads aligned to the region of interest and then uses AGE [40] to align this assembly to the appropriate region of the reference genome.
...
This alignment pipeline is largely inspired by the guidelines suggested by the authors of AGE in their newer LongAGE paper [41] and the authors of wtdbg2 [42] and consists of the following steps:

\begin{enumerate}
	\item For every insertion or deletion, Oxford Nanopore reads overlapping the region of interest are extracted from the NGMLR [24] BAM file. 
...
\item This assembly is polished using Oxford Nanopore data. For this, the same reads that were used for assembly are aligned to the consensus sequence using minimap2 [43] with options \texttt{-ax map-ont -r2k}. These aligned reads are used to update the consensus sequence with \texttt{wtpoa-cns}.
...
Although a newer version of AGE called LongAGE [41] requiring less memory has been recently released, we found it unsuitable for our needs due to cryptic error messages and slower speed.
...
In order to verify whether the pipeline indeed improved genotyping, we compared the genotyping precision and sensitivity of raw and refined SVs using three different genotypers: BayesTyper v. 1.5 [31], vg v. 1.23.0 [30], and Paragraph v. 2.4a [29].
...
In order to generate input datasets for genotyping, the SV sets obtained from the four samples were merged using SVmerge [45] with parameters \texttt{-maxdist 15 -reldist 0.1 -relsizediff 0.1 -relshift 0.1 -seqspecific}.
...
Following this, the VCF files were coordinate-sorted, normalized (using \texttt{bcftools norm}, [46]) and duplicate records were removed.
...
Briefly, k-mers were counted from the adapter-trimmed fastq files using KMC [47] and a Bloom filter was applied using the \texttt{bayesTyperTools makeBloom} utility.
...
The poor overall sensitivity of the results obtained with BayesTyper when genotyping Oxford Nanopore SVs is probably due to the small tolerance of this program to errors in SV breakpoint location or sequence content, as has been previously documented [30].
...
This is probably due to the high tolerance of Paragraph to errors in SV sequence (as documented in [29]) compared to the two other tools.
...
To study potential DNA transposable element (TE) activity in more detail, we extracted the SVs corresponding to all DNA TEs in the SoyTEdb database [48] that were matched by at least 3 SVs. 
...
We used the \texttt{PairwiseAlignment} function from the Bioconductor Biostrings package [49] to individually align all assemblies to a section of the reference genome ±~500 bp from the putative TE insertion site.
...
Multiple alignment was done with the MAFFT [50] suite of tools using the command \texttt{ginsi --reorder} which performs multiple global alignment.
...
We analyzed the insertion sequences using the GenericRepeatFinder software [51] with the following command:
...
The SNVs with FILTER = PASS among those discovered by Platypus [52] were extracted from the VCF file of the 102 samples sequenced by Illumina to perform this analysis.
The analysis of linkage disequilibrium (LD) using PLINK [53] in a 200-kb window surrounding the insertion location at Gm04:2,257,090 revealed a high-LD block spanning positions 2,220,398 to 2,259,326 that included the location of the insertion.
...
For each of the cultivars, we sampled 20\%, 40\%, 60\% and 80\% of the aligned reads using \texttt{samtools view --subsample} [46].
...
\begin{table}
	...
	\begin{threeparttable}\small
		...
		\item[d] Bonferroni-corrected p-values computed using the conditional hypergeometric test in the GOstats R package [54]
		\end{tablenotes}
	\end{threeparttable}
\end{table}
\end{landscape}
...
\begin{table}
	...
	\begin{threeparttable}\small
		...
		\item[d] Bonferroni-corrected p-values computed using the hypergeometric test in the GOstats R package [54]
		\end{tablenotes}
	\end{threeparttable}
\end{table}
\end{landscape}
...
\begin{cfigure}
	...
	\caption[Heterozygosity rates of the population of 102 Canadian soybean cultivars]{
		...
		Heterozygous calls in the context of inbred lines may occur due to various causes including genotype-calling errors in low-depth samples (as shown here), copy number variants generating spurious heterozygous genotype calls [6], or genuine residual heterozygosity.
	}
