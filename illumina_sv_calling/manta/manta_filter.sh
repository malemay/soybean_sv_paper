...
# Manta was run in 10 randomly selected batches of 10 or 11 samples
...
# DEPENDENCY : The candidateSV.vcf.gz files of the ten batches used to call Manta
# Looping over the 10 samples
for i in $(seq 1 10)
do
	manta_file=manta_sample${i}/MantaWorkflow/results/variants/candidateSV.vcf.gz
...
for i in $(seq 1 10)
do
	$btools convertAllele -g ../../refgenome/Gmax_508_v4.0_mit_chlp.fasta -v manta${i}_SV_filtered.vcf \
...
# DEPENDENCY : illumina_sv_calling/manta/ACO_header_line.txt
...
../../scripts/extract_svs_50.awk manta_merged.vcf | \
	...
	$bcftools annotate --set-id "%INFO/ACO\_%INFO/SVTYPE\_%ID" -Ov - > manta_svs.vcf
