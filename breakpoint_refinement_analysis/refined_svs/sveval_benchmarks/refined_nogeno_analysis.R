...
for(i in names(line_names)) {
	...
	# DEPENDENCY : refined reference SVs called with Sniffles, filtered and normalized
	...
	for(j in c("paragraph", "bayestyper", "vg")) {
		...
		# DEPENDENCY : refined SVs called by BayesTyper, Paragraph and vg
		if(j == "bayestyper") {
...
# OUTPUT : breakpoint_refinement_analysis/refined_svs/sveval_benchmarks/nogeno_RData/sveval_nogeno_rates.RData
